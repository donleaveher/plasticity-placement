...
RECOVERY_AUTHORIZATION_SCHEMA_VERSION = "p0d2hrtb-zero-artifact-recovery-authorization-v1"
RECOVERY_CLAIM_SCHEMA_VERSION = "p0d2hrtb-zero-artifact-recovery-claim-v1"
...
def inspect_zero_artifact_recovery(
    ...
    *,
    ...
    if age_seconds < MIN_STALE_SECONDS:
        raise PermissionError("running RTB attempt is not stale enough for recovery")
    ...
    if not isinstance(identity, dict):
        raise ValueError("RTB manifest has no preregistered identity")
    _verify_preflight(output_dir, identity)
    cpr_output = Path(str(identity["cpr_output"]))
    qualification_summary = Path(str(identity["qualification_summary"]))
    observed_source_snapshot = _source_snapshot(cpr_output, qualification_summary.parent)
    if observed_source_snapshot != identity.get("source_snapshot"):
        raise ValueError("RTB source artifacts changed before zero-artifact recovery")
    ...
    if (
        ...
        raise ValueError("original RTB authorization is missing or changed")
    original_authorization = read_json_object(
        original_authorization_path, "original RTB authorization"
    )
    if original_authorization.get("decision") != "approved":
        raise ValueError("original RTB authorization is not approved")
    ...
    evidence = {
        ...
        "source_snapshot_sha256": json_hash(observed_source_snapshot),
...
def recovery_authorization_template(
    ...
    *,
    ...
    return {
        ...
        "experiment_code_revision_lock_sha256": inspection["experiment_code_revision_lock_sha256"],
...
def recover_zero_artifact_attempt(
    ...
    *,
    ...
    static_claim = {
        ...
        "experiment_code_revision_lock_sha256": template["experiment_code_revision_lock_sha256"],
    ...
    manifest = _load_manifest(output_dir)
    if manifest.get("state") != "running":
        raise PermissionError("RTB state changed during zero-artifact recovery")
...
def _validate_completed_recovery(
    ...
    if not adopted_path.is_file() or not claim_path.is_file():
        raise ValueError("authorized recovery is missing its immutable governance artifacts")
