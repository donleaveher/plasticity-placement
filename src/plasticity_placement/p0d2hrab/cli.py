...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="运行 frozen receipt/action binding audit")
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("environment")
    plan = commands.add_parser("plan")
    plan.add_argument("--output", type=Path, required=True)
    plan.add_argument("--rsh-output", type=Path, required=True)
...
def main() -> None:
    ...
    if args.command == "plan":
        path = plan_audit(output_dir=args.output, rsh_output=args.rsh_output)
        ...
        manifest = read_json_object(args.output / "manifest.json", "receipt/action manifest")
        print(
            json.dumps(
                {
