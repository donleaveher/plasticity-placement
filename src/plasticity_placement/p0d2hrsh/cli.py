...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="运行 frozen CPR route-state handoff audit")
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("environment")
    plan = commands.add_parser("plan")
    plan.add_argument("--output", type=Path, required=True)
    plan.add_argument("--rtb-output", type=Path, required=True)
...
def main() -> None:
    ...
    if args.command == "plan":
        path = plan_audit(output_dir=args.output, rtb_output=args.rtb_output)
    elif args.command == "authorize":
        path = authorize_audit(args.output, args.authorization)
    elif args.command == "run":
        path = run_audit(args.output)
    elif args.command == "status":
        manifest = read_json_object(args.output / "manifest.json", "handoff manifest")
        print(
            json.dumps(
                {
