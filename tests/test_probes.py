...
def test_p0c_parser_rejects_missing_or_ambiguous_actions() -> None:
    actions = ("act_n7", "act_p3")
    assert parse_unique_action("act_n7", actions) == "act_n7"
    assert parse_unique_action("none", actions) is None
    assert parse_unique_action("act_n7 then act_p3", actions) is None
    assert parse_unique_action("maybe act_n7", actions) is None
    assert parse_unique_action("act_n7 act_n7", actions) is None
    assert parse_unique_action("act_n7.", actions) is None
    assert parse_unique_action(" ACT_N7\n", actions) == "act_n7"
